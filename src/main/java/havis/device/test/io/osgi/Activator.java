package havis.device.test.io.osgi;
...
import havis.device.test.io.StubHardwareManager;
...
public class Activator implements BundleActivator {
	...
	public void start(BundleContext bundleContext) throws Exception {
		...
		hwMgmtTracker = new ServiceTracker<HardwareMgmt, HardwareMgmt>(ctx, HardwareMgmt.class, null) {

			@Override
			public HardwareMgmt addingService(ServiceReference<HardwareMgmt> reference) {
			...
			@Override
			public void removedService(ServiceReference<HardwareMgmt> reference, HardwareMgmt service) {
	...
	protected void registerService(BundleContext bundleContext) {
		if (serviceReg != null)
			return;
		this.serviceReg = bundleContext.registerService(IODevice.class.getName(), new ServiceFactory<IODevice>() {
			@Override
